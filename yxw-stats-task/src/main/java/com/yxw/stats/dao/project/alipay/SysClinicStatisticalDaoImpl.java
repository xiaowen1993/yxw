...
@Repository("alipaySysClinicStatisticalDao")
public class SysClinicStatisticalDaoImpl extends BaseDaoImpl<SysClinicStatistical, String> implements SysClinicStatisticalDao {
	...
	private final static String BATCH_INSERT_CLINIC_STATS_DATA = "batchInsertClinicStatsData";
	...
	public String findCurrClinicStatsMaxDate(String hospitalId) {
		...
		try {
			...
			logger.error(String.format("查询项目平台支付宝门诊统计的最大日期出错！语句：%s", getSqlName(SQLNAME_FIND_CURR_CLINIC_STATS_MAXDATE)), e);
			throw new SystemException(String.format("查询项目平台支付宝门诊统计的最大日期出错！语句：%s", getSqlName(SQLNAME_FIND_CURR_CLINIC_STATS_MAXDATE)), e);
		}
		return result;
	}
	...
	public List<SysClinicStatistical> findClinicStatsData(Map<String, Object> params) {
		...
		try {
			...
			logger.error(String.format("查询项目平台支付宝门诊统计数据出错！语句：%s", getSqlName(SQLNAME_FIND_CLINIC_STATS_DATA)), e);
			throw new SystemException(String.format("查询项目平台支付宝门诊统计数据出错！语句：%s", getSqlName(SQLNAME_FIND_CLINIC_STATS_DATA)), e);
		}
		return list;
	}
	...
	public void batchInsertClinicStatsData(List<SysClinicStatistical> entitys) {
		try {
			...
			sqlSession.insert(getSqlName(BATCH_INSERT_CLINIC_STATS_DATA), params);
		} catch (Exception e) {
			logger.error(String.format("批量插入对象出错！语句：%s", getSqlName(BATCH_INSERT_CLINIC_STATS_DATA)), e);
			throw new SystemException(String.format("批量插入对象出错！语句：%s", getSqlName(BATCH_INSERT_CLINIC_STATS_DATA)), e);
		}
	}
}