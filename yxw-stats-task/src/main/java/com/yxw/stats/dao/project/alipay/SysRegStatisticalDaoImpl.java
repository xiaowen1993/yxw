...
@Repository("alipaySysRegStatisticalDao")
public class SysRegStatisticalDaoImpl extends BaseDaoImpl<SysRegStatistical, Long> implements SysRegStatisticalDao {
	...
	private final static String BATCH_INSERT_REGISTER_STATS_DATA = "batchInsertRegisterStatsData";
	...
	public String findCurrRegisterStatsMaxDate(String hospitalId) {
		...
		try {
			...
			logger.error(String.format("查询项目平台支付宝挂号统计的最大日期出错！语句：%s", getSqlName(SQLNAME_FIND_CURR_REGISTER_STATS_MAXDATE)), e);
			throw new SystemException(String.format("查询项目平台支付宝挂号统计的最大日期出错！语句：%s", getSqlName(SQLNAME_FIND_CURR_REGISTER_STATS_MAXDATE)), e);
		}
		return result;
	}
	...
	public List<SysRegStatistical> findRegisterStatsData(Map<String, Object> params) {
		...
		try {
			...
			logger.error(String.format("查询项目平台支付宝挂号统计数据出错！语句：%s", getSqlName(SQLNAME_FIND_REGISTER_STATS_DATA)), e);
			throw new SystemException(String.format("查询项目平台支付宝挂号统计数据出错！语句：%s", getSqlName(SQLNAME_FIND_REGISTER_STATS_DATA)), e);
		}
		return list;
	}
	...
	public void batchInsertRegisterStatsData(List<SysRegStatistical> entitys) {
		try {
			...
			sqlSession.insert(getSqlName(BATCH_INSERT_REGISTER_STATS_DATA), params);
		} catch (Exception e) {
			logger.error(String.format("批量插入对象出错！语句：%s", getSqlName(BATCH_INSERT_REGISTER_STATS_DATA)), e);
			throw new SystemException(String.format("批量插入对象出错！语句：%s", getSqlName(BATCH_INSERT_REGISTER_STATS_DATA)), e);
		}
	}
