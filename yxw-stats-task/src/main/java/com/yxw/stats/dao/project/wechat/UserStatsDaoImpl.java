...
public class UserStatsDaoImpl extends BaseDaoImpl<UserStats, Long> implements UserStatsDao {
	...
	public String findCurrUserStatsMaxDate(String hospitalId) {
		...
		try {
			...
			logger.error(String.format("查询项目平台微信用户关注数统计的最大日期出错！语句：%s", getSqlName(SQLNAME_FIND_CURR_USER_STATS_MAXDATE)), e);
			throw new SystemException(String.format("查询项目平台微信用户关注数统计的最大日期出错！语句：%s", getSqlName(SQLNAME_FIND_CURR_USER_STATS_MAXDATE)), e);
		}
		return result;
