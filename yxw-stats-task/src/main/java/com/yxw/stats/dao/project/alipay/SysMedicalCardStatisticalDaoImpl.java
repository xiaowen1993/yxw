...
@Repository("alipaySysMedicalCardStatisticalDao")
public class SysMedicalCardStatisticalDaoImpl extends BaseDaoImpl<SysMedicalCardStatistical, Long> implements SysMedicalCardStatisticalDao {
	...
	private final static String BATCH_INSERT_MEDICALCARD_STATS_DATA = "batchInsertMedicalCardStatsData";
	...
	public String findCurrMedicalCardStatsMaxDate(String hospitalId) {
		...
		try {
			...
			logger.error(String.format("查询项目平台支付宝绑卡统计的最大日期出错！语句：%s", getSqlName(SQLNAME_FIND_CURR_MEDICALCARD_STATS_MAXDATE)), e);
			throw new SystemException(String.format("查询项目平台支付宝绑卡统计的最大日期出错！语句：%s", getSqlName(SQLNAME_FIND_CURR_MEDICALCARD_STATS_MAXDATE)),
					e);
		}
		return result;
	}
	...
	public List<SysMedicalCardStatistical> findMedicalCardStatsData(Map<String, Object> params) {
		...
		try {
			...
			logger.error(String.format("查询项目平台支付宝绑卡统计数据出错！语句：%s", getSqlName(SQLNAME_FIND_MEDICALCARD_STATS_DATA)), e);
			throw new SystemException(String.format("查询项目平台支付宝绑卡统计数据出错！语句：%s", getSqlName(SQLNAME_FIND_MEDICALCARD_STATS_DATA)), e);
		}
		return list;
	}
	...
	public void batchInsertMedicalCardStatsData(List<SysMedicalCardStatistical> entitys) {
		try {
			...
			sqlSession.insert(getSqlName(BATCH_INSERT_MEDICALCARD_STATS_DATA), params);
		} catch (Exception e) {
			logger.error(String.format("批量插入对象出错！语句：%s", getSqlName(BATCH_INSERT_MEDICALCARD_STATS_DATA)), e);
			throw new SystemException(String.format("批量插入对象出错！语句：%s", getSqlName(BATCH_INSERT_MEDICALCARD_STATS_DATA)), e);
		}
	}
}