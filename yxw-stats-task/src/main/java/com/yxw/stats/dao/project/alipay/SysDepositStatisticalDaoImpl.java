...
@Repository("alipaySysDepositStatisticalDao")
public class SysDepositStatisticalDaoImpl extends BaseDaoImpl<SysDepositStatistical, Long> implements SysDepositStatisticalDao {
	...
	private final static String BATCH_INSERT_DEPOSIT_STATS_DATA = "batchInsertDepositStatsData";
	...
	public String findCurrDepositStatsMaxDate(String hospitalId) {
		...
		try {
			...
			logger.error(String.format("查询项目平台支付宝住院押金统计的最大日期出错！语句：%s", getSqlName(SQLNAME_FIND_CURR_DEPOSIT_STATS_MAXDATE)), e);
			throw new SystemException(String.format("查询项目平台支付宝住院押金统计的最大日期出错！语句：%s", getSqlName(SQLNAME_FIND_CURR_DEPOSIT_STATS_MAXDATE)), e);
		}
		return result;
	}
	...
	public List<SysDepositStatistical> findDepositStatsData(Map<String, Object> params) {
		...
		try {
			...
			logger.error(String.format("查询项目平台支付宝住院押金统计数据出错！语句：%s", getSqlName(SQLNAME_FIND_DEPOSIT_STATS_DATA)), e);
			throw new SystemException(String.format("查询项目平台支付宝住院押金统计数据出错！语句：%s", getSqlName(SQLNAME_FIND_DEPOSIT_STATS_DATA)), e);
		}
		return list;
	}
	...
	public void batchInsertDepositStatsData(List<SysDepositStatistical> entitys) {
		try {
			...
			sqlSession.insert(getSqlName(BATCH_INSERT_DEPOSIT_STATS_DATA), params);
		} catch (Exception e) {
			logger.error(String.format("批量插入对象出错！语句：%s", getSqlName(BATCH_INSERT_DEPOSIT_STATS_DATA)), e);
			throw new SystemException(String.format("批量插入对象出错！语句：%s", getSqlName(BATCH_INSERT_DEPOSIT_STATS_DATA)), e);
		}
	}
}