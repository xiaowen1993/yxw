...
import com.yxw.stats.task.project.callable.DepositStatisticalCollectCall;

public class RegisterStatisticalCollector {

	public static Logger logger = LoggerFactory.getLogger(RegisterStatisticalCollector.class);
	...
	public void start() {
		...
		if (threadNum > 0) {
			// 设置线程池的数量
			ExecutorService collectExec = Executors.newFixedThreadPool(threadNum, new SimpleThreadFactory("depositStatistical"));
			List<FutureTask<String>> taskList = new ArrayList<FutureTask<String>>();

			for (String platformType : CommConstants.PLATFORMTYPES) {
				DepositStatisticalCollectCall collectCall = new DepositStatisticalCollectCall(platformType);
				// 创建每条指令的采集任务对象
