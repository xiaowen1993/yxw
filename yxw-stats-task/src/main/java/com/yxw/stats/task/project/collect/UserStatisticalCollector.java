...
import com.yxw.stats.task.project.callable.UserStatisticalCollectCall;

public class UserStatisticalCollector {

	public static Logger logger = LoggerFactory.getLogger(UserStatisticalCollector.class);
	...
	public void start() {
		...
		if (threadNum > 0) {
			// 设置线程池的数量
			ExecutorService collectExec = Executors.newFixedThreadPool(threadNum, new SimpleThreadFactory("userStatistical"));
			List<FutureTask<String>> taskList = new ArrayList<FutureTask<String>>();

			for (String platformType : CommConstants.PLATFORMTYPES) {
				UserStatisticalCollectCall collectCall = new UserStatisticalCollectCall(platformType);
				// 创建每条指令的采集任务对象
