...
import com.yxw.stats.task.project.callable.MedicalCardStatisticalCollectCall;

public class MedicalCardStatisticalCollector {

	public static Logger logger = LoggerFactory.getLogger(MedicalCardStatisticalCollector.class);
	...
	public void start() {
		...
		if (threadNum > 0) {
			// 设置线程池的数量
			ExecutorService collectExec = Executors.newFixedThreadPool(threadNum, new SimpleThreadFactory("medicalCardStatistical"));
			List<FutureTask<String>> taskList = new ArrayList<FutureTask<String>>();

			for (String platformType : CommConstants.PLATFORMTYPES) {
				MedicalCardStatisticalCollectCall collectCall = new MedicalCardStatisticalCollectCall(platformType);
				// 创建每条指令的采集任务对象
