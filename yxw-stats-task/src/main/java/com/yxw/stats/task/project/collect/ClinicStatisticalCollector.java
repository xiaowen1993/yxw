...
import com.yxw.stats.task.project.callable.ClinicStatisticalCollectCall;

public class ClinicStatisticalCollector {

	public static Logger logger = LoggerFactory.getLogger(ClinicStatisticalCollector.class);
	...
	public void start() {
		...
		if (threadNum > 0) {
			// 设置线程池的数量
			ExecutorService collectExec = Executors.newFixedThreadPool(threadNum, new SimpleThreadFactory("clinicStatistical"));
			List<FutureTask<String>> taskList = new ArrayList<FutureTask<String>>();

			for (String platformType : CommConstants.PLATFORMTYPES) {
				ClinicStatisticalCollectCall collectCall = new ClinicStatisticalCollectCall(platformType);
				// 创建每条指令的采集任务对象
