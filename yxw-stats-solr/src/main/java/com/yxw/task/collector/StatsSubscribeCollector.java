...
import com.yxw.task.callable.StatsSubscribeCallable;
...
public class StatsSubscribeCollector implements Collector {
	...
	public void startUp() {
		ExecutorService executorService = Executors.newFixedThreadPool(PoolConstant.threadNum, new SimpleThreadFactory("statsSubscribe"));
		...
		try {
			...
			e.printStackTrace();
			logger.error("关注信息统计错误. errorMsg: {}. cause: {}.", new Object[] { e.getMessage(), e.getCause() });
		} finally {
