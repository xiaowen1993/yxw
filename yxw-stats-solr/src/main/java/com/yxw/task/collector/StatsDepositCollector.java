...
import com.yxw.task.callable.StatsDepositCallable;
	...
	public void startUp() {
		...
		for (Entry<String, List<StatsHospitalInfosVo>> areaMap : map.entrySet()) {
			...
			for (StatsHospitalInfosVo infosVo : areaMap.getValue()) {
				StatsDepositCallable callable = new StatsDepositCallable(infosVo, false, statsDate);
		...
		try {
			...
			if (taskList.size() > 0) {
				...
				for (Entry<String, List<StatsHospitalInfosVo>> areaMap : map.entrySet()) {
					...
					StatsDepositCallable callable = new StatsDepositCallable(areaMap.getValue().get(0), true, statsDate);
					FutureTask<String> task = new FutureTask<String>(callable);
					taskList.add(task);
					executorService.submit(task);
				}
			...
			e.printStackTrace();
			logger.error("押金订单统计错误. errorMsg: {}. cause: {}.", new Object[] { e.getMessage(), e.getCause() });
		} finally {
