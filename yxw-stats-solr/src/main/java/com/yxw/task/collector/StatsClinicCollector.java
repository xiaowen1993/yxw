...
import com.yxw.task.callable.StatsClinicCallable;
	...
	public void startUp() {
		...
		for (Entry<String, List<StatsHospitalInfosVo>> areaMap : map.entrySet()) {
			...
			for (StatsHospitalInfosVo infosVo : areaMap.getValue()) {
				StatsClinicCallable callable = new StatsClinicCallable(infosVo, false, statsDate);
		...
		try {
			...
			if (taskList.size() > 0) {
				...
				for (Entry<String, List<StatsHospitalInfosVo>> areaMap : map.entrySet()) {
					...
					StatsClinicCallable callable = new StatsClinicCallable(areaMap.getValue().get(0), true, statsDate);
					FutureTask<String> task = new FutureTask<String>(callable);
					taskList.add(task);
					executorService.submit(task);
				}
			...
			e.printStackTrace();
			logger.error("门诊订单统计错误. errorMsg: {}. cause: {}.", new Object[] { e.getMessage(), e.getCause() });
		} finally {
