...
import com.yxw.task.callable.StatsRegCallable;
import com.yxw.vo.StatsHospitalInfosVo;

public class StatsRegCollector implements Collector {

	private Logger logger = LoggerFactory.getLogger(StatsRegCollector.class);

	public void startUp() {
		ExecutorService executorService = Executors.newFixedThreadPool(PoolConstant.threadNum, new SimpleThreadFactory("statsReg"));
...
//				if (infosVo.getHospitalCode().equals("gzhszhyy")) {
					StatsRegCallable callable = new StatsRegCallable(infosVo, false, statsDate);
		...
		try {
			...
			if (taskList.size() > 0) {
				...
				for (Entry<String, List<StatsHospitalInfosVo>> areaMap : map.entrySet()) {
					...
					StatsRegCallable callable = new StatsRegCallable(areaMap.getValue().get(0), true, statsDate);
					FutureTask<String> task = new FutureTask<String>(callable);
					taskList.add(task);
					executorService.submit(task);
				}
			...
			e.printStackTrace();
			logger.error("挂号订单统计错误. errorMsg: {}. cause: {}.", new Object[] { e.getMessage(), e.getCause() });
		} finally {
