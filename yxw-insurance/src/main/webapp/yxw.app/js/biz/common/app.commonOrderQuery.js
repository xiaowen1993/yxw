var recordQuery = {
	init: function() {
		...
		var userNameFilter = getCookie("userNameFilterVal");
		if (userNameFilter) {
			var selectedUserNameFilter= $("#userNameFilter").find("option[value='"+userNameFilter+"']");
			if (selectedUserNameFilter) {
				$('#userNameFilter').val(userNameFilter);
				recordQuery.selectScreen(document.getElementById("userNameFilter"));
			}
		}
		
		if($('#userNameFilter').val()!=""&&$('#hospitalFilter').val()!=""){
			getDataList();
		}
	...
	bindEvent: function() {
		$('#userNameFilter').off('change').on('change', function() {
			recordQuery.selectScreen(this);
			setCookie("userNameFilterVal", $(this).val());
			if($('#userNameFilter').val()!=""&&$('#hospitalFilter').val()!=""){
				getDataList();
			}
