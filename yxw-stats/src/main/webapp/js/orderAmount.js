var order = {
	...
	getDatas: function(options) {
		...
		// options.title += $('#statsTypeList>li[data-value="' +
		// options.dateCode + '"]>a').text();
	...
	addInit: function(objId) {
		var types = [ 1, 2, 1, 2, 1, 2, 1, 2, 1, 2 ];
		var titles = [ '订单金额概要图', '订单金额概要表', '各类型订单金额图', '各类型订单金额表', '各类型订单金额占比图', '各类型订单金额占比表', '各渠道订单金额图',
				'各渠道订单金额表', '各渠道订单金额占比图', '各渠道订单金额占比表' ];
		var clazzs = [ 'orderChart', 'orderTable', 'orderTypeChart', 'orderTypeTable', 'orderTypeScaleChart',
				...
				'orderPlatformScaleTable' ];
		var hints = [ '所有挂号、门诊、押金业务的订单金额总和（含已支付、未支付） -- 单位：元', '所有挂号、门诊、押金业务的订单金额总和（含已支付、未支付） -- 单位：元', '挂号、门诊、押金业务的订单金额（含已支付、未支付） -- 单位：元',
				'挂号、门诊、押金业务的订单金额（含已支付、未支付） -- 单位：元', '挂号、门诊、押金业务的订单金额占比（含已支付、未支付） -- 单位：元', '挂号、门诊、押金业务的订单金额占比（含已支付、未支付） -- 单位：元', '微信、支付宝平台所有订单金额（含已支付、未支付） -- 单位：元',
				'微信、支付宝平台所有订单金额（含已支付、未支付） -- 单位：元', '微信、支付宝平台所有订单金额占比（含已支付、未支付） -- 单位：元', '微信、支付宝平台所有订单金额占比（含已支付、未支付） -- 单位：元' ];
	...
	addByOrder: function(objId, data) {
		...
		$.each(data.yData, function(key, value) {
			sBody += '<tr><td>' + key + '</td>';
			$.each(yxw.fromFenToYuan(value), function(itemKey, itemValue) {
				sBody += '<td>' + yxw.number_format(itemValue, 2) + '</td>';
			});
	...
	addByOrderTypeScale: function(objId, data) {
		...
		sHead += '<tr><th>订单类型</th><th>订单总金额</th></tr>';

		var seriesDatas = [];
		var legendDatas = [];
		data.values = yxw.fromFenToYuan(data.values);
		for (var i = 0; i < data.legend.length; i++) {
			sBody += '<tr>';
			sBody += '	<td>' + data.legend[i] + '</td>';
			sBody += '	<td>' + yxw.number_format(data.values[i], 2) + '</td>';
			sBody += '</tr>';

			legendDatas.push(data.legend[i] + "：" + yxw.number_format(data.values[i]) + "元");
			seriesDatas.push({
				'name': data.legend[i] + "：" + yxw.number_format(data.values[i]) + "元",
				'value': data.values[i]
			});
		}
	...
	addByOrderPlatformScale: function(objId, data) {
		...
		sHead += '<tr><th>平台类别</th><th>订单总金额</th></tr>';
		...
		for (var i = 0; i < data.legend.length; i++) {
			...
			sBody += '	<td rowspan="1">' + yxw.number_format(data.values[i], 2) + '</td>';
			sBody += '</tr>';
	...
	addByRegTypeScale: function(objId, data) {
		...
		sHead += '<tr><th>挂号类别</th><th>订单总金额</th></tr>';
	...
	addByPaymentScale: function(objId, data) {
		...
		sHead += '<tr><th>支付类别</th><th>订单总金额</th></tr>';
