var card = {
	...
	loadInitData : function() {
		$.get('/common/getInitData', function(data) {
			if (data.status == 'OK' && data.message) {
				card.formatToolBar(data.message);
	...
	getDatas : function(options) {
		...
		if ($('#tab_' + tabId).length == 0) {
			...
			$.post('/card/getDatas', data, function(data) {
				if (data.status == 'OK') {
					card.formatData(contentId, data.message);
				}
			}, 'json');
		} else {
	...
	formatData : function(objId, data) {
		if ($('#' + objId).length != 0) {
			...
			case 1:
				...
				var titles = [ '累计绑卡数概要图', '各月份绑卡数概要图', '累计绑卡数图', '累计绑卡数表', '各月份绑卡数图', '各月份绑卡数表'];
				var clazzs = [ 'tCumulateCharts', 'tMonthCharts', 'cumulateCharts', 'cumulateTable', 'monthCharts', 'monthTable' ];
				$('#' + objId).html('').append(commonTabs.addContent(types, titles, clazzs));
				card.formatJustAreaData(objId, data);
				break;
			case 2:
				var types = [ 1,2,1,2 ];
				var titles = [ '累计绑卡数图', '累计绑卡数表', '各月份绑卡数图', '各月份绑卡数表'];
				var clazzs = [ 'cumulateCharts', 'cumulateTable', 'monthCharts', 'monthTable' ];
				$('#' + objId).html('').append(commonTabs.addContent(types, titles, clazzs));
				card.formatAllAreaAndHospitalsData(objId, data);
				break;
			case 3:
				var types = [ 1,2,1,2 ];
				var titles = [ '累计绑卡数图', '累计绑卡数表', '各月份绑卡数图', '各月份绑卡数表'];
				var clazzs = [ 'cumulateCharts', 'cumulateTable', 'monthCharts', 'monthTable' ];
				$('#' + objId).html('').append(commonTabs.addContent(types, titles, clazzs));
				card.formatOneAreaAndHospitalsData(objId, data);
				break;
			case 4:
				var types = [ 1,2,1,2 ];
				var titles = [ '累计绑卡数图', '累计绑卡数表', '各月份绑卡数图', '各月份绑卡数表'];
				var clazzs = [ 'cumulateCharts', 'cumulateTable', 'monthCharts', 'monthTable' ];
				$('#' + objId).html('').append(commonTabs.addContent(types, titles, clazzs));
				card.formatOneHospitalData(objId, data);
				break;
			}
			
			card.bindDownloadEvent();
		} else {
			console.log('没有tab页面.');
		}
	},
