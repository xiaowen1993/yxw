 ...
 * @Version: 1.0
 */
public class PreDayVisitWarnCollectCall implements Callable<RegisterRecord> {

	private VisitWarnHandleManager handler = SpringContextHolder.getBean(VisitWarnHandleManager.class);

	private Logger logger = LoggerFactory.getLogger(PreDayVisitWarnCollectCall.class);
	...
	public PreDayVisitWarnCollectCall(String hospitalCode, List<RegisterRecord> records) {
		super();
	...
	public RegisterRecord call() throws Exception {
		if (!CollectionUtils.isEmpty(records)) {
			logger.info("hospitalCode: {}, has {} tomorrow records to send...", new Object[] { hospitalCode, records.size() });
			handler.handlePreDateVisitWarn(hospitalCode, records);
			logger.info("hospitalCode: {}, today record warn done...", hospitalCode);
		} else {
			logger.info("hospitalCode: {}, has no records tomorrow...", hospitalCode);
		}
		return null;
	}
