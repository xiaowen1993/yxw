...
import com.yxw.task.callable.PreDayVisitWarnCollectCall;
 ...
 * @Version: 1.0
 */
public class PreDayvisitWarnCollector {
	public static Logger logger = LoggerFactory.getLogger(PreDayvisitWarnCollector.class);
	...
	public void start() {
		...
		if (CollectionUtils.isNotEmpty(results)) {
			...
			ConcurrentHashMap<String, CodeAndInterfaceVo> codeAndInterfaceMap = (ConcurrentHashMap<String, CodeAndInterfaceVo>) results.get(0);
		...
		if (CollectionUtils.isNotEmpty(results)) {
			...
			logger.info("加载第二天就诊的挂号记录...");
			Map<String, List<RegisterRecord>> map = registerService.findAllTomorrowRecords();
			logger.info("第二天就诊的挂号记录加载完毕...");
			...
			List<FutureTask<RegisterRecord>> taskList = new ArrayList<FutureTask<RegisterRecord>>();
			for (CodeAndInterfaceVo vo : codeAndInterfaceLs) {
				if (HospitalConstant.HOSPITAL_VALID_STATUS == vo.getStatus()) {
					...
					PreDayVisitWarnCollectCall collectCall = new PreDayVisitWarnCollectCall(hospitalCode, map.get(hospitalCode));
					// 创建每条指令的采集任务对象
					FutureTask<RegisterRecord> collectTask = new FutureTask<RegisterRecord>(collectCall);
			...
			try {
				List<RegisterRecord> updateRecord = new ArrayList<RegisterRecord>();
				for (FutureTask<RegisterRecord> taskF : taskList) {
					// 防止某个子线程查询时间过长 超过默认时间没有拿到抛出异常
					RegisterRecord record = taskF.get(Long.MAX_VALUE, TimeUnit.DAYS);
					if (record != null) {
