...
import com.yxw.task.callable.HospitalDoctorInfoCollectCall;
 ...
 * @Version: 1.0
 */
public class HospitalDoctorInfoCollector {
	public static Logger logger = LoggerFactory.getLogger(HospitalDoctorInfoCollector.class);
	...
	public void start() {
		...
		if (CollectionUtils.isNotEmpty(results)) {
			...
			if (threadNum > 0) {
				// 设置线程池的数量
				ExecutorService collectExec = Executors.newFixedThreadPool(threadNum, new SimpleThreadFactory("docotrInfo"));
				...
				for (CodeAndInterfaceVo vo : codeAndInterfaceLs) {
					if (HospitalConstant.HOSPITAL_VALID_STATUS == vo.getStatus()) {
						...
						HospitalDoctorInfoCollectCall collectCall =
								new HospitalDoctorInfoCollectCall(areaCode, hospitalId, hospitalName, hospitalCode, branchHospitalCode, interfaceId);
						// 创建每条指令的采集任务对象
				...
				for (FutureTask<HospitalBaseInfo> taskF : taskList) {
					...
					try {
						...
						logger.warn("HospitalDoctorInfoCollector get task result time out ,msg:{}.", e.getMessage());
					}
				}
