...
import com.yxw.task.callable.CurDayVisitWarnCollectCall;
 ...
 * @Version: 1.0
 */
public class CurDayvisitWarnCollector {
	public static Logger logger = LoggerFactory.getLogger(CurDayvisitWarnCollector.class);
	...
	public void start() {
		...
		if (CollectionUtils.isNotEmpty(results)) {
			...
			//			ConcurrentHashMap<String, CodeAndInterfaceVo> codeAndInterfaceMap = (ConcurrentHashMap<String, CodeAndInterfaceVo>) list.get(0);
			if (codeAndInterfaceLs.size() < threadNum) {
				threadNum = codeAndInterfaceLs.size();
			}

			logger.info("加载第二天就诊的挂号记录...");
			Map<String, List<RegisterRecord>> map = registerService.findAllTodayRecords();
			...
			for (CodeAndInterfaceVo vo : codeAndInterfaceLs) {
				if (HospitalConstant.HOSPITAL_VALID_STATUS == vo.getStatus()) {
					...
					CurDayVisitWarnCollectCall collectCall = new CurDayVisitWarnCollectCall(hospitalCode, map.get(hospitalCode));
					// 创建每条指令的采集任务对象
