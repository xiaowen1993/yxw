...
import com.yxw.task.callable.HandleClinicExceptionCollectCall;
 ...
 * @Version: 1.0
 */
public class HandleClinicExceptionCollector {
	public static Logger logger = LoggerFactory.getLogger(HandleClinicExceptionCollector.class);
	...
	public void start() {
		...
		if (CollectionUtils.isNotEmpty(results)) {
			...
			List<Object> jsonObjects = serveComm.get(CacheType.CLINIC_EXCEPTION_CACHE, "getClinicExceptionFromCache", params);

			if (jsonObjects.size() < threadNum) {
				threadNum = jsonObjects.size();
			}

			List<ClinicRecord> exceptionRecords = new ArrayList<ClinicRecord>(jsonObjects.size());

			if (threadNum > 0) {
				// 设置线程池的数量
				ExecutorService collectExec = Executors.newFixedThreadPool(threadNum, new SimpleThreadFactory("clinicExce"));
				List<FutureTask<ClinicRecord>> taskList = new ArrayList<FutureTask<ClinicRecord>>();
				logger.info("需处理异常订单的医院集合:{}", JSON.toJSONString(codeAndInterfaceLs));
				exceptionRecords = JSON.parseArray(JSON.toJSONString(jsonObjects), ClinicRecord.class);

				for (ClinicRecord record : exceptionRecords) {
					String keys =
					...
					if (vo != null) {
						...
						if (isFlag) {
							...
							HandleClinicExceptionCollectCall collectCall = new HandleClinicExceptionCollectCall(interfaceId, record);
							// 创建每条指令的采集任务对象
							FutureTask<ClinicRecord> collectTask = new FutureTask<ClinicRecord>(collectCall);
				...
				try {
					List<ClinicRecord> updateRecord = new ArrayList<ClinicRecord>();
					for (FutureTask<ClinicRecord> taskF : taskList) {
						// 防止某个子线程查询时间过长 超过默认时间没有拿到抛出异常
						ClinicRecord record = taskF.get(Long.MAX_VALUE, TimeUnit.DAYS);
						if (record != null) {
					...
					params.clear();
					params.add(startTime);
					serveComm.delete(CacheType.CLINIC_EXCEPTION_CACHE, "removeClinicExceptionFromCache", params);
				}
