...
<div id="body">
    ...
    <div id="paid-detail"></div>
     	...
     	<#if commonParams.appCode == 'innerChinaLife' && commonParams.isClaim !=1 >
		    <div class="btn-w">
		            <div class="btn btn-ok btn-block" id="toClinList">一键理赔</div>
		        </div>
		</#if>
...
<form id="voForm" method="post">
	...
	<input type="hidden" id="orderNo" name="orderNo" value="${commonParams.orderNo}" />
		...
		<input type="hidden" id="isClaim" name="isClaim" value="${commonParams.isClaim}" />
</form>
...
<script type="text/javascript" src="${basePath}yxw.app/js/biz/clinic/app.paidDetail.js"></script>
...
<script>
	...
	//提交
	    $('#toClinList').off('click').on('click', function() {
	        <#-- var action="http://pt.yx129.cn:8009/api/claimType"; -->
	        var action="http://localhost:8009/api/claimType";
