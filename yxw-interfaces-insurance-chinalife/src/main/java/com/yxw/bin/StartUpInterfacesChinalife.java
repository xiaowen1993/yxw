...
public class StartUpInterfacesChinalife {
	...
	private static void loadSystemConfig() {
		...
		PropertiesUtils releasePropertiesUtils =
				new PropertiesUtils("classpath:" + SystemConstants.FILE_SEPARATOR + SystemConfig.getStringValue("release"));
		Enumeration<Object> releaseKeys = releasePropertiesUtils.getProperties().keys();
		while (releaseKeys.hasMoreElements()) {
			String key = releaseKeys.nextElement().toString();
			String value = releasePropertiesUtils.getProperty(key);
			SystemConfig.put(key, value);
		}
	}
