...
(function($, undefined) {
	...
	function Confirm(options) {
		...
		var cancel = {
			title : that.options.cancel.title,
			fun : that.options.cancel.fun
		};
		var ok = {
			title : that.options.ok.title,
			fun : that.options.ok.fun
		};
		...
		if (!$modal.length) {
			$modal = $('<div class="modal" id="'
					...
					+ ' <a href="#" class="btn btn-ok" data-action="1">'
					+ ok_title + '</a>' + '</div>' + '</div>')
					.appendTo(this.options.appendTo).hide();
			if (typeof this.options.ok == 'object') {
				var self = this;
				$modal.find('.btn-ok').attr('href', '#').on(
			...
			if (typeof this.options.cancel == 'object') {
				var self = this;
				$modal.find('.cancel').attr('href', '#').on(
						'click.' + pluginName, function(e) {
							e.preventDefault();
	...
	$.fn[pluginName] = function(options) {
		return this.each(function() {
					...
					if (!(obj = $.data(this, pluginName))) {
						var $this = $(this), data = $this.data(), title = $this
								.attr('title')
								|| data.title, opts = $.extend({},
