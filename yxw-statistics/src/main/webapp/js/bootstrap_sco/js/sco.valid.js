...
(function($, undefined) {
	...
	$.extend(Valid.prototype, {
		// this is the main function - it returns either true if the validation
		// passed or false
		validate : function() {
			var self = this, form_fields = this.$form.serializeArray(), all_fields = this.$form
					.find(':input[name]').map(function() {
								return this.name;
							}).get();
			...
			$.each(this.errors, function(field_name, error) {
						...
						if (self.options.wrapper !== null) {
							$input.closest(self.options.wrapper)
									.removeClass('error');
						}
					});
			this.errors = {};
			...
			$.each(self.options.rules, function(field_name, rules) {
				var field = null, normalized_rules = {};
				...
				// if field was not found, it could mean 2 things: mispelled
				// field name in the rules or the field is not a successful
				// control
				...
				// if it's still null then it's either mispelled or disabled. We
				...
				if (field !== null) {
					$.each(rules, function(rule_idx, rule_value) {
						...
						if ($.type(rule_value) === 'string') {
							...
							// if not string then we assume it's a {key: val}
							// object. Only 1 key is allowed
							$.each(rule_value, function(k, v) {
										// make sure the requested method
										// actually exists.
					...
					$.each(normalized_rules, function(fn_name, fn_args) {
						...
						if (self.methods[fn_name].call(self, field.name,
								field.value, fn_args, normalized_rules) !== true) {
							self.errors[field.name] = self.format.call(self,
									field.name, fn_name, fn_args);
						}
					});
				}
			});
		...
		show : function(errors) {
			...
			$.each(errors, function(k, v) {
				var $input = self.$form.find('[name="' + k + '"]'), $span = $input
						.siblings('.message');
				if (self.options.wrapper !== null) {
		...
		methods : {
			...
			matches : function(field, value, param) {
				return value === this.$form.find('[name="' + param + '"]')
						.val();
			}
		},
		...
		/**
		 * finds the most specific error message string and replaces any
		 * ":value" substring with the actual value
		 */
		format : function(field_name, rule, params) {
			var message;
			if (typeof this.options.messages[field_name] !== 'undefined'
					&& typeof this.options.messages[field_name][rule] !== 'undefined') {
			...
			if ($.type(params) !== 'undefined' && params !== null) {
				if ($.type(params) === 'boolean' || $.type(params) === 'string'
						|| $.type(params) === 'number') {
					params = {
						value : params
					};
	...
	/**
	 * main function to use on a form (like $('#form).scojs_valid({...})).
	 * Performs validation of the form, sets the error messages on form inputs
	 * and returns true/false depending on whether the form passed validation or
	 * not
	 * 
	 * @param {hash|string}
	 *            options the hash of rules and messages to validate the form
	 *            against (and messages to show if failed validation) or the
	 *            string "option"
	 * @param {string}
	 *            key the option key to retrieve or set. If the third param of
	 *            the function is available then act as a setter, otherwise as a
	 *            getter.
	 * @param {mixed}
	...
	$.fn[pluginName] = function(options, key, value) {
		return this.each(function(idx, form) {
			var $form = $(form), validator = $form.data(pluginName);
			if ($.type(options) === 'object') {
				if (!validator) {
					var opts = $.extend({}, $.fn[pluginName].defaults, options,
							$form.data());
					validator = new Valid($form, opts);
					$form.data(pluginName, validator).attr('novalidate',
							'novalidate');
				}
				$form.ajaxForm({
						...
						if (response.status === 'fail') {
							...
							if (typeof options.onError !== 'function'
									|| options.onError.call(this, response,
											validator, $form) !== false) {
								$.scojs_message(response.message,
							...
							if (typeof options.onSuccess !== 'function'
								...
								if (response.data.next) {
									if (response.data.next === '.') { // refresh
																		// current
																		// page
										window.location.href = window.location.href
												.replace(/#.*$/, '');
									} else if (response.data.next === 'x') { // close
																				// the
																				// parent
																				// modal
										$form.closest('.modal')
								...
								if (response.data.message) {
									$.scojs_message(response.data.message,
	...
	$.fn[pluginName].defaults = {
		wrapper : 'label' // the html tag that wraps the field and which
							// defines a "row" of the form
		,
		rules : {} // array of rules to check the form against. Each value
					// should be either a string with the name of the method to
					// use as rule or a hash like {method: <method params>}
		,
		messages : {}
		// custom error messages like {username: {not_empty: 'hey you forgot to
		// enter your username', min_length: 'come on, more than 2 chars, ok?'},
		// password: {....}}
	};
})(jQuery);
