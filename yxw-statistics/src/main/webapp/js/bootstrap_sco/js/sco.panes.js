...
(function($, undefined) {
	...
	function Panes($wrapper, options) {
		...
		var transitionEnd = ($.support.transition && this.options.easing)
		...
		this.$pane_wrapper.on('select.' + pluginName, function(e, options,
						index) {
					...
					function onEnd() {
						self.$panes.eq(options.active).removeClass('active '
								+ direction);
						self.options.active = index;
						self.$panes.eq(index).removeClass([type, direction]
					...
					self.$panes.eq(index).addClass(type)[0].offsetWidth; // force
																			// reflow
		...
		select : function(index) {
			if (index !== this.options.active) {
				if (typeof this.options.onBeforeSelect != 'function'
						|| this.options.onBeforeSelect.call(this, index) !== false) {
					this.$pane_wrapper.trigger('select.' + pluginName, [
									this.options, index]);
					return true;
				}
			}
