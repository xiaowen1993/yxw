...
(function($, undefined) {
	...
	$[pluginName] = function(message, type, time) {
		clearTimeout($[pluginName].timeout);
		var $selector = $('#' + $[pluginName].options.id);
		if (!$selector.length) {
			$selector = $('<div/>', {
						id : $[pluginName].options.id
					}).appendTo($[pluginName].options.appendTo);
		}
		...
		if (type == undefined || type == 'error') {
			$selector.removeClass($[pluginName].options.okClass)
					.addClass($[pluginName].options.errClass);
		} else if (type == 'ok') {
			$selector.removeClass($[pluginName].options.errClass)
					.addClass($[pluginName].options.okClass);
		}
		if (time == undefined) {
			var delay = 2000;
		} else {
	...
	$.extend($[pluginName], {
				options : {
					...
					animate : 2000,
					appendTo : 'body' // where should the modal be appended to
										// (default to document.body). Added for
										// unit tests, not really needed in real
										// life.
				},
