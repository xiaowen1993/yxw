...
function eventStream(request, response) {
	...
	var lastEventId = Number(request.headers["last-event-id"])
			|| Number(parsedURL.query.lastEventId) || 0;
	var authorization = request.headers["authorization"];

	if (parsedURL.query.authorization !== undefined && !authorization) {
		response.writeHead(401, {
					"WWW-Authenticate" : "Basic realm=\"EventSource"
							+ (1 + Math.random()).toString().slice(2) + "\"",
	...
	function sendMessages() {
		...
		while (lastEventId - firstId < history.length) {
			response.write("id: "
					...
					+ "\n"
					+ "data: "
					+ (history[lastEventId - firstId]).replace(/[\r\n\x00]/g,
							"\ndata: ") + "\n\n");
			lastEventId += 1;
		}
		response.write(":\n");
	...
	response.socket.setTimeout(0); // see
									// http://contourline.wordpress.com/2011/03/30/preventing-server-timeout-in-node-js/
	...
	if (estest !== undefined) {
		...
		estest.slice(0, i).replace(/[^\n]*/g, function(line) {
					...
					if (header[0].trim() !== "") {
						headers[header[0].trim()] = header.slice(1).join(":")
								.trim();
					}
				});
...
function onRequest(request, response) {
	...
	if (query.message) {
		time = new Date();
		data = "[" + time.toISOString() + "][IP: "
				+ request.connection.remoteAddress + "] " + query.message;
		response.writeHead(200, {
	...
	if (pathname === "/events") {
		...
		var files = ["/example.html", "/nodechat.css", "/eventsource.js",
				"/nodechat.js", "/tests.html", "/qunit.css", "/qunit.js",
				"/tests.js", "/stop.js"];
		...
		fs.stat(__dirname + pathname, function(error, stats) {
			if (error) {
				...
				var mtime = Date.parse(request.headers["if-modified-since"])
						|| 0;
				if (stats.mtime <= mtime) {
					...
					response.writeHead(200, {
								"Content-Type" : (pathname.indexOf(".js") !== -1
										? "text/javascript"
