...
(function(global) {
	...
	var Transport = isCORSSupported ? XHR : (XDR !== undefined
			? XDR
			: undefined);
	var WAITING = -1;
	var CONNECTING = 0;
	...
	function EventSource(url, options) {
		...
		var withCredentials = isCORSSupported && options !== undefined
		...
		function onEvent(type) {
			var responseText = currentState === OPEN
					|| currentState === CONNECTING ? xhr.responseText : "";
			var event = undefined;
			...
			if (currentState === CONNECTING) {
				...
				if (status === 0 && statusText === "" && type === "load"
						&& responseText !== "") {
					status = 200;
					statusText = "OK";
					if (contentType === "") { // Opera 12
						var tmp = (/^data\:([^,]*?)(?:;base64)?,[\S]*$/)
								.exec(url);
					...
					if (state === AFTER_CR && c === "\n") {
						...
						if (c === "\r" || c === "\n") {
							if (field === "data") {
								...
								initialRetry = getDuration(Number(value),
										initialRetry);
								retry = initialRetry;
							} else if (field === "heartbeatTimeout") {
								heartbeatTimeout = getDuration(Number(value),
										heartbeatTimeout);
								if (timeout !== 0) {
									clearTimeout(timeout);
			...
			if ((currentState === OPEN || currentState === CONNECTING)
					&& (type === "load" || type === "error"
							...
							|| (charOffset > 1024 * 1024) || (timeout === 0 && !wasActivity))) {
		...
		onTimeout = function() {
			...
			if (isXHR) {
				// improper fix to match Firefox behaviour, but it is better
				// than just ignore abort
			...
			if (s !== "data:" && s !== "blob:") {
				s = url
						+ ((url.indexOf("?", 0) === -1 ? "?" : "&")
								+ "lastEventId="
								+ encodeURIComponent(lastEventId) + "&r=" + (Math
			...
			if (isXHR) {
				// withCredentials should be set after "open" for Safari and
				// Chrome (< 19 ?)
				...
				// "Cache-control: no-cache" are not honored in Chrome and
				// Firefox
	...
	var isEventSourceSupported = function() {
		if (global.EventSource !== undefined) {
			try {
				...
				es.close();
				return es.withCredentials === false && es.url !== ""; // to
																		// filter
																		// out
																		// Opera
																		// 12
