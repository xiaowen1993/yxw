...
/*
 * global setTimeout, clearTimeout, location, EventSource, asyncTest, ok,
 * strictEqual, start
 */
...
(function(global) {
	...
	var url = "http://" + location.hostname + ":" + String(location.port)
			+ "/events";
	var url4CORS = "http://" + location.hostname + ":"
			+ (String(location.port) === "8004" ? "8003" : "8004") + "/events";
	var urlWithAuthorization = "http://" + "user012:pass012@"
			+ location.hostname + ":" + String(location.port) + "/events";

	var commonHeaders = "Access-Control-Allow-Origin: *\n"
			+ "Content-Type: text/event-stream\n" + "Cache-Control: no-cache\n";

	// try with native EventSource in Opera 12
	asyncTest("URL with username and password for Basic Authorization",
			function() {
				var es = new EventSource(urlWithAuthorization
	...
	asyncTest("blob: URL", function() {
				var es = new EventSource(global.URL
						.createObjectURL(new global.Blob(
								["retry:1000\ndata:1\n\n"], {
	...
	asyncTest("data: URL", function() {
				var es = new EventSource("data:text/event-stream;charset=utf-8,"
	...
	asyncTest("EventSource + window.stop", function() {
		...
		es.onmessage = es.onerror = es.onopen = function(event) {
	...
	/*
	 * asyncTest("EventSource + file download", function () { var es = new
	 * EventSource(url + "?estest=" + encodeURIComponent(commonHeaders + "\n\n" +
	 * "data:1\n<delay(500)>")); location.href = url + "?estest=" +
	 * encodeURIComponent(commonHeaders + "Content-Disposition: attachment;
	 * filename=\"test.txt\"" + "\n\n" + "test"); setTimeout(function () {
	 * ok(es.readyState !== EventSource.CLOSED, es.readyState); start(); },
	 * 2000); });
	 */
	asyncTest("EventSource constructor", function() {
				var body = "";
				var es = new EventSource(url + "?estest="
						+ encodeURIComponent(commonHeaders + "\n\n" + body));
	...
	asyncTest("EventSource ping-pong", function() {
				...
				var body = "retry: 500\n" + "data: " + Math.random() + "\n\n"
						+ "<delay(1500)>" + "data: " + Math.random() + "\n\n"
						+ "<delay(1500)>" + "data: " + Math.random() + "\n\n"
						+ "<delay(10000)>";
				if (global.XDomainRequest) {
					body = new Array(2048).join(" ") + body;
				}
				es = new EventSource(url + "?estest="
						+ encodeURIComponent(commonHeaders + "\n\n" + body));
				es.onmessage = function() {
					++n;
					clearTimeout(timer);
	...
	/*
	 * // Chrome: 0 // Opera, Firefox: 03 // IE 9-10: 023 // EventEmitter
	 * node.js: 023
	 * 
	 * asyncTest("EventTarget addEventListener/removeEventListener", function () {
	 * var body = "data: test\n\n"; var es = new EventSource(url + "?estest=" +
	 * encodeURIComponent(commonHeaders + "\n\n" + body)); var s = ""; var
	 * listeners = {}; function a(n) { return listeners[n] || (listeners[n] =
	 * function () { s += n; if (n === 0) { es.removeEventListener("message",
	 * a(0)); es.removeEventListener("message", a(2));
	 * es.addEventListener("message", a(4)); setTimeout(function () {
	 * es.close(); strictEqual(s, "03", "EventTarget"); start(); }, 0); } }); }
	 * es.addEventListener("message", a(0)); es.addEventListener("message",
	 * a(1)); es.addEventListener("message", a(2));
	 * es.addEventListener("message", a(3)); es.removeEventListener("message",
	 * a(1)); });
	 */
	...
	// Native EventSource + CORS: Opera 12, Firefox 11, Chrome 26 (WebKit
	// 537.27)
	asyncTest("EventSource CORS", function() {
				var body = "id: <lastEventId(100)>\n" + "data: 0\n\n";
				var es = new EventSource(url4CORS + "?estest="
						+ encodeURIComponent(commonHeaders + "\n\n" + body));
	...
	asyncTest("event-stream with \"message\", \"error\", \"open\" events",
			function() {
				var body = "data: a\n\n" + "event: open\ndata: b\n\n"
						+ "event: message\ndata: c\n\n"
						+ "event: error\ndata: d\n\n" + "event:\ndata: e\n\n"
	...
	asyncTest("infinite reconnection", function() {
				var es = new EventSource("http://functionfunction"
						+ Math.floor(Math.random() * 1e10) + ".org");
				var n = 0;
				es.onerror = function() {
					++n;
					if (es.readyState === EventSource.CLOSED) {
