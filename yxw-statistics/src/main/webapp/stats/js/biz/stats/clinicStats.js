var clinicStats = {
	bindEvent: function() {
		$('.tab').off('click').on('click', function() {
			if (!$(this).is('.active')) {
				...
				clinicStats.showOtherStats($(this));
			}
		});
		...
		$('#btnSearch').off('click').on('click', function() {
			...
			// 重置分页数据
			clinicStats.resetPageInfos();
			
			clinicStats.getStatsInfos();
		});
	...
	getStatsInfos: function() {
		...
		var url = appPath + "stats/clinic/getStatsInfo";
		...
		$.ajax({
			type : 'POST',  
	       	...
	       	success: function(data) {
	       		console.log(data);
	       		if (data.status == 'OK') {
	       			clinicStats.formatStatsInfo(data.message);
	       		} else {
	       			clinicStats.showNoDatas($('#statsDetail'), 6);
	       		}
	       	}
		});
	},
