...
<#include "./sys/common/hospital_setting.ftl">
    <div id="content-header">
        <div class="widget-title"><h3 class="title"><#if flagTime==null><a  href="${request.contextPath}/message/reply?method=toFocusedReply&hospitalId=${hospitalId}"  class="green">返回</a>　｜　</#if>编辑单图文消息</h3></div>
    </div>
                  	...
                  	<form id="frm">
	                    <div class="t-margin">
	                        <div class="t-area">
	                          <i class="caret"></i>
	                          <div class="inner">
	                              <div class="control-group"><h4 class="title">标题</h4><div class="controls"><input name="title" class="span6 js_title" type="text"  oninput="OnInput(event)" onpropertychange="OnPropChanged(event)"  value="${mixedMeterial.title}"/></div></div>
	                              <div class="control-group"><h4 class="title">作者<small class="green">(选填)</small></h4><div class="controls"><input class="span6" type="text" value="${mixedMeterial.author}" name="author"/></div></div>
	                              <div class="control-group"><h4 class="title">封面<small class="green">(大图尺寸建议900像素 * 500像素)</small></h4>
	                                  <div class="controls">
	                                        ...
	                                        <div class="btn-library" onclick="$msg.invokeMixMetaLib('${hospitalId}','single');">调用素材库</div>
	                              ...
	                              <div class="control-group"><h4 class="title">摘要<small class="green">(选填)</small></h4>
	                                  <div class="controls">
	                                      <textarea class="span12 js_desc" rows="3" name="description">${mixedMeterial.description}</textarea>
	                                  </div>
	                              </div>
	                              ...
	                              <div class="control-group"><h4 class="title">添加原文链接<small class="green">（选填）</small></h4><div class="controls"><input class="span6" type="text" value="${mixedMeterial.link}" name="link"/></div></div>
	                          </div>
	                      	</div>
	                        <div class="space20"></div>
	                        <button type="button" class="btn btn-save w130" onclick="editSaveSingle('${flagTime}');" >保存</button>
	                    </div>
                    </form>
                </div>
            </div>
...
<script language="javascript">
    ...
    //保存编辑内容
    function editSaveSingle(flagTime)
    {
    	editor.sync();
	    $.ajax({
			...
			success:function(resp)
			{
				if(resp.status=='OK')
				{
					if(flagTime != null && flagTime != ''){
						var edittime=new Date(resp.message.et);
						var etstr=formatterDateTime(edittime);
						window.opener.singleText(1,resp.message.id,resp.message.title,resp.message.description,resp.message.coverPicPath,flagTime,etstr);
					}else{
						window.opener.addSingleUnit(1,resp.message.id,resp.message.title,resp.message.content,resp.message.coverPicPath);
					}
					window.close();
				}
