...
<script type="text/javascript">
	...
	//    html+='<div class="operation" onclick="delKeyWord(this);"><i class="icons-rule icons-del-grey"></i></div></div></div>';
	  ...
	  //  html+='<div class="operation grey" onclick="addPicKeyword('+ruleNum+');"><i class="icons-rule icons-pic"></i>图片</div>';
	    ...
	    html+='<button type="button" class="btn btn-remove" onclick="del_addRule(this);" >取消</button>';
	    html+='</div><div class="space10"></div></div></div></form></div>';
	...
	function editRule(ruleId,ruleName,ruleType,kwList,replyList){
	  ...
	  //  html+='<div class="operation grey" onclick="addPicKeyword('+ruleNum+');"><i class="icons-rule icons-pic"></i>图片</div>';
	    ...
	    html+='</div><div class="space10"></div></div></div><div class="space20"></div></form></div>';
		...
		$.ajax({  
            ...
            success: function(resp) {  
        		if(resp.status=='OK')
        		{
					...
					$(obj).parents('.widget-content').next().remove();
    ...
    function addPicKeyword(index){
        boxPic=new $Y.dialog({
            ...
            callback:function(){
                $.ajax({
                    url:path+'/message/reply?method=toDialogPic',
                    dataType:'html',
                    cache:false,
                    success:function(html){
                        boxPic.content(html);
