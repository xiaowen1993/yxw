...
$terminal.device.toAdd = function(hospitalId) {
	location.href = "toAdd?hospitalId=" + hospitalId;
}

$terminal.device.save = function(hospitalId) {
	var nullHint = $terminal.checkNull();
	if (nullHint) {
		alert(nullHint);
		return;
	}
	...
	$.ajax({
    	...
    	success : function(data) {
    		console.dir(data);
    		if (data.status == 'OK') {
    			location.href = 'device?hospitalId='+hospitalId;
    		} else {
    			alert(data.message);
    		}
    	}
...
$terminal.getBasePath = function() {
	return $("#basePath").val();
}


$terminal.changePage = function(pageNum, pageSize) {
  if (pageNum) {
