	...
	public String call() throws Exception {
		...
		if (statsArea) {
			if (StringUtils.isBlank(statsMonth)) {
				logger.info("[标准平台关注数统计]统计日期为空，区域不为空，默认为初始化统计所有日期");
				return service.statsAreaInfos(request);
			} else {
				logger.info("[标准平台关注数统计]统计日期为{}.区域不为空.", statsMonth);
			...
			if (StringUtils.isBlank(statsMonth)) {
				logger.info("[标准平台关注数统计]统计日期为空，区域为空，默认为初始化统计所有日期");
				return service.statsInfos(request);
			} else {
				logger.info("[标准平台关注数统计]统计日期为{}. 区域为空.", statsMonth);
				request.setStatsMonth(statsMonth);
				request.setBeginDate(statsMonth);
