...
public class StatsClinicCallable implements Callable<String> {
	...
	public StatsClinicCallable(Integer platform, String hospitalCode, String branchCode, String statsDate) {
		super();
	...
	public String call() throws Exception {
		...
		if (platform != -1) {
			if (StringUtils.isBlank(statsDate)) {
				...
				return clinicService.statsInfosWithinPlatform(request);
			} else {
			...
			if (StringUtils.isBlank(statsDate)) {
				...
				return clinicService.statsInfosWithoutPlatform(request);
			} else {
				logger.info("[全平台统计]统计日期为{}. 单独开启一天的统计.", statsDate);
				request.setStatsDate(statsDate);
				return clinicService.statsInfoForDayWithoutPlatform(request);
			}
		}
