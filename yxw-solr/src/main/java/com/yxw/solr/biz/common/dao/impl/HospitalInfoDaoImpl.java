package com.yxw.solr.biz.common.dao.impl;
...
public class HospitalInfoDaoImpl extends BaseDaoImpl<HospitalInfosVo, String> implements HospitalInfoDao {
	...
	private final static String SQL_NAME_GET_STD_INFOS = "getStdInfos";
	...
	public List<StatsHospitalInfosVo> getStdInfos() {
		try {
			return sqlSession.selectList(getSqlName(SQL_NAME_GET_STD_INFOS));
		} catch (Exception e) {
			logger.error(String.format("获取标准平台医院出错！语句：%s", getSqlName(SQL_NAME_GET_STD_INFOS)), e);
			throw new SystemException(String.format("获取标准平台医院出错！语句：%s", getSqlName(SQL_NAME_GET_STD_INFOS)), e);
		}
	}
