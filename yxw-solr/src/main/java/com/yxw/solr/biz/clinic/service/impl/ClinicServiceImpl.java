...
public class ClinicServiceImpl implements ClinicService {
	...
	public YxwResponse getStatsInfos(ClinicStatsRequest request) {
		...
		try {
			...
			logger.error("获取门诊统计信息异常. errorMsg: {}. cause: {}.", new Object[]{e.getMessage(), e.getCause()});
	...
	private void saveRebuildInfos(ClinicStatsRequest request, Collection<Object> collection) {
		StringBuffer sb = new StringBuffer();
		sb.append("hospitalCode").append(SolrConstant.COLON).append(request.getHospitalCode()).append(SolrConstant.AND);
		sb.append("branchCode").append(SolrConstant.COLON).append(request.getBranchCodeValue()).append(SolrConstant.AND);
		sb.append("platform").append(SolrConstant.COLON).append(request.getPlatformValue());
	...
	private void saveDailyInfos(ClinicStatsRequest request, Collection<Object> collection) {
		StringBuffer sb = new StringBuffer();
		sb.append("hospitalCode").append(SolrConstant.COLON).append(request.getHospitalCode()).append(SolrConstant.AND);
		sb.append("branchCode").append(SolrConstant.COLON).append(request.getBranchCodeValue()).append(SolrConstant.AND);
	...
	public String statsInfoForDayWithinPlatform(ClinicStatsRequest request) {
		...
		ClinicStatsRequest yesterdayRequest = new ClinicStatsRequest();
	 ...
	 * @return
	 */
	private String getStartDate(ClinicStatsRequest request) {
		...
		try {
			...
			if (request.getPlatform() != -1) {
				...
				QueryResponse queryResponse = solrClient.query(Cores.CORE_STATS_CLINIC, solrQuery);
				SolrDocumentList documentList = queryResponse.getResults();
				if (CollectionUtils.isNotEmpty(documentList)) {
			...
			logger.error("门诊统计获取开始时间异常. errorMsg: {}. cause: {}.", new Object[]{e.getMessage(), e.getCause()});
		} 

		return startDate;
	}

	private Map<String, Map<String, Object>> getPartRefundStatsInfos(ClinicStatsRequest request) {
		...
		try {
			...
			logger.error("门诊统计获取部分退费信息异常. errorMsg: {}. cause: {}.", new Object[]{e.getMessage(), e.getCause()});
		} 

		return result;
	}

	private Map<String, Map<String, Object>> getPayStatsInfos(ClinicStatsRequest request) {
		Map<String, Map<String, Object>> result = new HashMap<String, Map<String, Object>>();
		...
		try {
			...
			logger.error("门诊统计获取已支付订单异常. errorMsg: {}. cause: {}.", new Object[]{e.getMessage(), e.getCause()});
		}

		return result;
	}

	private Map<String, Map<String, Object>> getRefundStatsInfos(ClinicStatsRequest request) {
		Map<String, Map<String, Object>> result = new HashMap<String, Map<String, Object>>();
		...
		try {
			...
			logger.error("门诊统计获取已退费订单异常. errorMsg: {}. cause: {}.", new Object[]{e.getMessage(), e.getCause()});
		} 
	...
	private Map<String, Map<String, Integer>> getAllStats(String hospitalCode, String branchCode, String statsDate, String beginDate) {
		...
		if (StringUtils.isNotBlank(beginDate)) {
			...
			sbFilter.append("statsDate").append(SolrConstant.COLON).append(SolrConstant.INTERVAL_START)
					.append(beginDate).append(SolrConstant.TO).append(SolrConstant.ALL_VALUE)
